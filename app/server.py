...
def ensure_model(config: TsuzuriConfig) -> Path:
    ...
    downloaded = hf_hub_download(repo_id=config.model.repo_id, filename=config.model.gguf_file)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not config.model.server_url:
        if args.auto_setup and not (config.model.llama_bin or args.llama_bin):
